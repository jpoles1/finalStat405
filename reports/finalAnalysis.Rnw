...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Each state is assigned two Senators, while the number of representatives for a state in the House of Representatives is based on the state's population.
...
The United States Senate maintains an archive of roll-call votes - those votes in which the vote of each individual senator is recorded, rather than just having an overall tally. This archive is accessible via a series of folders on their servers containing XML files. We parsed these XML files using the code found in Appendix A in order to gain insights into the types of bills reviewed in the US Senate, and the patterns of bill passage.
\subsection{What Types of Roll-Call Votes are Handled by the US Senate?}
Various bills are introduced to the Senate, but most fall into broad categories. After plotting counts of bills, we found that two categories of bills account for most bills in the Senate: motions, where a vote is called in order to direct the proceedings of the Senate (end discussion on a topic or consider a bill), and ammendements in which the senators vote on additions/removals of text from bills. Both of these categories has a count of more than 3500. All other kinds of bills only count to about 1400 in total, so we synthesized them into one distinct category: ”Others”.
...
After examining the overall distribution of bill types in Congresses 101-113, we investigated trends in the changing number of bills of the three bill types (see 4.1) over time. We expected that bill types distribution would be fairly stable over time, but this shows the counts of the two majority categories (”Amendment” and ”Motion”) are randomly oscillating, indicating the lack of a clear pattern. However, in the majority of years examined, between 100 and 200 bills were “Amendment” and “Motion”, while typically under 100 bills were in the category “Other”, which is consistent with our previous plot. In addition, there are peaks for both ”Amendment” and ”Motion” in 1995, indicating that the Senate was particularly busy that year, perhaps as a consequence of the 1995 Republican Revolution (see 5.2). Also, in 2014, the count of bills regarding amendments dropped significantly, while the count of other bills increased significantly, perhaps due to Obamacare debates during that year.

<<tidy=FALSE, echo=FALSE, out.height="3.2in", out.width="8in", fig.width=8, fig.height=3.2>>==
...
Most bills initiated in the Senate are either passed or rejected. One would think that the percentage of bills passed would be fairly stable over time but the statistics suggest otherwise: the percentage of passed bills consistently, but not significantly, decreased between Congress 107 and Congress 113. However, the percentage of passed bills soared in Congress 113. This may be explained as the effect of the debate on Obamacare or other important policies. The majority of passed bills may be explained by the fact that most bills are routine or do not have a serious impact (ex. changing the name of a national park).

<<tidy=FALSE, echo=FALSE, out.height="4in", out.width="8in", fig.width=8, fig.height=4, cache=TRUE>>==
...
In accord with popular belief, we show that the United States Senate is a bipartisan political body. The Republican and Democratic parties constantly grapple for majority seats in the Senate, while smaller parties like the Independents try to hold on to the few seats they can win.
...
There are several advantages to having a two-party system. Bipartisan governments generally tend to be more efficient and more easily managed than those with multiple parties, and two-party system may be the reason for the majority of bills being passed in the US Senate (4.3). In addition, a two-party system intuitively makes the voting system less complicated for voters. However, with only two options, voters may not entirely agree with either party. Gridlock is a major concern, as is corruption.
...
The table in 3.4 suggests the possibility of corruption in the US Senate through nepotism. In a country of 318 million individuals, the objective likelihood of relatives serving in the Senate is incredibly low, but the data shows that the phenomenon does in fact occur. Further analysis into individuals’ voting patterns on specific votes, the effects lobbying, and other political persuasion can gain more insight into the true extent of corruption in the US government.
...
<<eval=FALSE>>=
  votedata = queryDB("SELECT id, party, vote, count(*) as ct, year 
                     FROM votes 
                     WHERE party in ('D', 'R') 
                     GROUP BY id, vote, year")
  votedatawide = dcast(votedata, id+year+party~vote, value.var="ct")
  votedatawide$diff = (votedatawide$Yea-votedatawide$Nay)
  ggplot(votedatawide, aes(x=as.factor(year), y=diff))+
    geom_boxplot()+
...
@
\newpage
\subsection{Appendix C: Code for Generating Animated .gifs for Time Series Vizualization}

During the course of our analysis, we were struggling to find means of vizualizing time-scale state-level data like change in average missed votes by senators of each state over time; or state political preference over time. The typical chloropleth map (where each state is filled with a color determined by a variable) only allows for two dimensions to be vizualized: state, and variable level. Should we wish to use time we might instead have to use averages over time (which eliminates some resolution of the data during exploration).

In \href{https://github.com/jpoles1/finalStat405/blob/master/figures/mapSeries/series.R}{series.R} we present a simple technique which generates (using the ggmap package) and saves sequential chloropleth maps. These maps are then stitched together by one of any number of applications in order to create an animated gif (we use the ImageMagick toolkit, which comes pre-installed on Xubuntu). For example, we will look at senator miss rate by state over time:

\subsubsection{First we generate a series of plots and save them to disk:}

<<eval=FALSE>>==
stateMap = map_data("state")
congressRange = 101:113
alldata = queryDB("SELECT state as stateAbrev, avg(missed_votes_pct) as missed, congressNumber 
                  FROM members  
                  GROUP BY congressNumber, state 
                  ORDER BY congressNumber DESC, missed DESC")
upperbound = ceiling(mean(alldata$missed)+(2*sd(alldata$missed)))
for(congressNumber in congressRange){
  stateVote = queryDB(sprintf("
    SELECT state as stateAbrev, avg(missed_votes_pct) as missed 
    FROM members 
    WHERE congressNumber=%s 
    GROUP BY state",congressNumber))
...
\subsubsection{Then we generate the gif:}

Execute \href{https://github.com/jpoles1/finalStat405/blob/master/figures/mapSeries/animate.sh}{animate.sh}
This file should be in the folder containing the generated images. The file contains the following command:
<<eval=FALSE>>==
  convert -delay 100 -loop 0 *.png map.gif
@

\subsubsection{\href{https://raw.githubusercontent.com/jpoles1/finalStat405/master/figures/mapSeries/missed/map.gif}{View the final product here!}}
