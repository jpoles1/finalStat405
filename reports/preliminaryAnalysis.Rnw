...
@
\subsection{Introduction to US Political Parties}
...
@
\subsection{Common Senatorial Surnames}
...
@
\newpage
\subsection{Examining State Party Preferences}
...
<<out.height="2in", out.width="8in", fig.width=8, fig.height=2, echo=FALSE, eval=FALSE>>==
  ...
  ggplot()+
    ...
    scale_y_continuous(breaks=NULL)
@
\subsection{Indepdendent Senators}
