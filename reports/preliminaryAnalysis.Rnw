...
<<tide=TRUE>>=
  ...
  ggplot(mediaPlotData, aes(x=congressNumber, y=value, color=Party))+geom_point()+geom_line()+facet_wrap(~variable)+xlim(101, 113)+xlab("Congress Number")+ylab("Proportion of Senators in Party")+ggtitle("Web Platform Usage By Senators")
@
\newpage
<<tidy=TRUE>>=
...
<<>>=
  ...
  c = ggplot(partyDataByYear, aes(x=congressNumber, y=ct, fill=Party))+scale_fill_manual(name = "Party", values = c("#4169E1", "green", "#FF0000"))+geom_histogram(position="fill", stat="identity", width=1)+xlab("Congress Number\n")+scale_x_continuous(breaks=101:113)+ylab("\nPercentage of Senate Seats")+ggtitle("Senate Seats Held Per Party By Year\n")
  grid.newpage()
