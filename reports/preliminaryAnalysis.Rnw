...
Two supplementary R files are included with this analysis. These contain the R code used to fetch the data (divided by filetype) from our three sources:
\begin{enumerate}
  \item congressAPI.R - This Script fetches all of the datasets and stores them in a SQLite database file (data.sqlite). The fetching and handling JSON files from 
\end{enumerate}
\end{document}