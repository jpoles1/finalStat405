...
\begin{enumerate}
  \item The Senate provides a more balanced overview of the political sentiments present on the state scale.
  \begin{enumerate}
    \item Each state is assigned 2 Senators, while the House of Representatives determines the number of representatives for a state based upon population.
    \item The Senate is smaller in size, less susceptible to partisan issues, and bueracratic gridlock
  \end{enumerate}
...
The NY Times Congress API provides an interface for accessing a number of characteristics on US Senators including their web presence, party and certain voting patterns. This provides us some introductory insights into the politicians elected by the US citizenry to represent them in the federal government.
...
To begin our analysis, we turn to the use of web platforms by Senators. These platforms (including websites, Facebook, and Twitter) have become more common over time. Since the inception of FDR's fireside chats, electronic media has permitted mass communication of political thought; this capacity has been greatly expanded by the interenet. In our plot we display lifetime usage of each platform. We show that usage has grown rapidly for all platforms, though Facebook has lagged behind. High web-platform usage is not surprising, given the number of senators that host their websites under senate.gov sub-domains (name.senate.gov). Twitter also seems to be popular, and is often utilized to disseminate political ideology and news.\footnote{For instance: \href{https://twitter.com/McConnellPress}{https://twitter.com/McConnellPress}}
...
Most senators serve no more than 5 terms. Democrats and Republicans have similar distributions, though Democrats have a peak at greater than 10 terms.
...
In the previous section, we examined the overall changes in bill passage in the US Senate. For each of the roll-call votes we examinine in our analysis, there is a corresponding list of each Senator's vote: Yea or Nay. More content needed here...
...
@
\newpage
\subsection{How large is the disparity between yea votes by each major party?}
Content needed here? Figure may need reworking.

<<echo=FALSE, out.height="4.5in", out.width="8in", fig.width=8, fig.height=4.5, cache=TRUE, warning=FALSE>>=
  query = "select r.year as year, r.voteNumber as voteNumber,
              (r.c - d.c) * 1.0 / (r.c + d.c) as diff 
...
<<echo=FALSE, out.height="4.5in", out.width="8in", fig.width=8, fig.height=4.5, cache=TRUE, warning=FALSE>>=
query = "select r.year as year, r.voteNumber as voteNumber,
              r.c * 1.0 / (r.c + d.c) as rpercent, d.c * 1.0 / (r.c + d.c) as dpercent 
           from (select voteNumber, year, count(*) as c
                 from votes 
                 where vote == 'Yea' and party == 'R' group by year, voteNumber)
                 as r
                join
                 ...
                 as d
	              on r.voteNumber == d.voteNumber and r.year == d.year"
  yeaPercentage = queryDB(query, "data.sqlite")
  RepYeaPercentageMean = setNames(aggregate(rpercent ~ year, yeaPercentage, mean), c("year", "mean"))
  RepYeaPercentageMean$party = rep(c('Republican'), times=length(RepYeaPercentageMean))
  DemYeaPercentageMean = setNames(aggregate(dpercent ~ year, yeaPercentage, mean), c("year", "mean"))
  DemYeaPercentageMean$party = rep(c('Democratic'), times=length(DemYeaPercentageMean))
  yeaPercentageMean = rbind(RepYeaPercentageMean, DemYeaPercentageMean)
  ggplot(yeaPercentageMean) +
  aes(x = year, y = mean, group = party, colour = party) +
  scale_x_discrete(breaks=c(1989,1994,1999,2004,2009,2014)) +
  labs(title="Average Percentage of 'yea' Votes of two majority parties 
       \nin total 'yea' votes from 1989 to 2014") +
  xlab("Year") +
  ylab("Percentage of 'yea' votes") +
  geom_line() +
  geom_point()  
  @
